...
const STATUS_OPTIONS: StatusPedido[] = [
  'Aguardando Separação', 'Em Separação', 'Em Conferência', 'Alteração de Pedido',
  'Aguardando Compra', 'Faturamento', 'Em Rota', 'Entregue', 'Cancelado'
];
...
export function AcoesPedido({ pedido }: { pedido: Pedido }) {
  ...
  const canAdvance = hasRole('admin') || hasRole('logistica') || hasRole('gestor') || hasPermissao('pode_avancar_etapa');
  ...
  const mutation = useMutation({
    ...
    onMutate: async (newStatus) => {
      ...
      // Optimistic update
      qc.setQueryData(['pedidos', 'operacional'], (old: any) => {
        if (!old) return old;
        return old.map((p: any) => p.id === pedido.id ? { ...p, status_atual: newStatus } : p);
      });
      return { prev };
    },
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="sm" variant="outline" className="w-full mt-2">Avançar Etapa</Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
