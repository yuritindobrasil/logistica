...
export function LinhaDoTempoObservacoes({ pedido }: { pedido: Pedido }) {
  ...
  const { data: observacoes, isLoading } = useQuery<ObservacaoOperacional[]>({
    ...
    queryFn: async () => {
      ...
      return (data ?? []) as ObservacaoOperacional[];
    },
  });
              ...
              <div key={obs.id} className="bg-secondary/40 p-2 rounded-md text-xs space-y-1">
                <div className="flex items-center justify-between font-semibold text-muted-foreground">
                  ...
                  <span className="flex items-center gap-1">
                    <Clock className="h-3 w-3" /> {new Date(obs.criado_em).toLocaleString("pt-BR")}
                  </span>
                </div>
        ...
        <Dialog open={modalOpen} onOpenChange={setModalOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Adicione anotações operacionais sobre o pedido {pedido.numero_pedido_externo}.
              </DialogDescription>
            </DialogHeader>
            <Textarea
              value={novaObs}
