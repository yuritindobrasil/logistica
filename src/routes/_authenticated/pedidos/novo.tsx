...
function NovoPedidoPage() {
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors } } = useForm<FormValues>({
    ...
    defaultValues: { numero_pedido_externo: '', cliente_id: '', valor_total: 0, observacoes_iniciais: '' },
  });
  ...
  const { data: clientes, isLoading: isLoadingClientes } = useQuery({
    queryKey: ['clientes'],
    queryFn: async () => {
      const { data, error } = await supabase.from('clientes' as never).select('id, razao_social, codigo_cliente').order('razao_social');
      ...
      >
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            ...
            <Input id="numero_pedido_externo" placeholder="Ex: 123456" {...register('numero_pedido_externo')} />
            {errors.numero_pedido_externo && <p className="mt-1 text-xs text-destructive">{errors.numero_pedido_externo.message}</p>}
          </div>

          <div>
            <Label htmlFor="valor_total">Valor Total (R$) *</Label>
            <Input id="valor_total" type="number" step="0.01" min="0" placeholder="0.00" {...register('valor_total')} />
        ...
        <div>
          ...
          <Select value={clienteIdWatcher} onValueChange={(v) => setValue('cliente_id', v, { shouldValidate: true })}>
          ...
          {errors.cliente_id && <p className="mt-1 text-xs text-destructive">{errors.cliente_id.message}</p>}
        </div>

        <div>
          <Label htmlFor="observacoes_iniciais">Observações Iniciais</Label>
          <Textarea id="observacoes_iniciais" placeholder="Anotações para a logística..." {...register('observacoes_iniciais')} className="min-h-[100px]" />
          {errors.observacoes_iniciais && <p className="mt-1 text-xs text-destructive">{errors.observacoes_iniciais.message}</p>}
        </div>
