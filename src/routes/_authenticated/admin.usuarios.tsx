...
import { ROLE_LABELS, ROLES, type AppRole, type PermissoesIndividuais, type UsuarioPerfil } from '@/types/domain';
...
const PERM_KEYS: Array<{ key: keyof Omit<PermissoesIndividuais, 'id' | 'usuario_id' | 'updated_at'>; label: string }> = [
...
function UsuariosPage() {
  ...
  const filtered = useMemo(() => {
    return (usuarios ?? []).filter((u) => {
      const matchBusca = !busca || u.nome_completo.toLowerCase().includes(busca.toLowerCase()) || u.email.toLowerCase().includes(busca.toLowerCase());
  ...
  const roleMutation = useMutation({
    mutationFn: async ({ id, role }: { id: string; role: AppRole }) => {
      const { error } = await supabase.from('usuarios_perfis' as never).update({ role } as never).eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">Visão Administrativa — Roles e Permissões Individuais.</p>
      </header>

      <div className="flex flex-wrap items-center gap-3 rounded-lg border border-border bg-card p-4">
        <div className="relative flex-1 min-w-[240px]">
          <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <Input placeholder="Buscar por nome ou e-mail" value={busca} onChange={(e) => { setBusca(e.target.value); setPage(0); }} className="pl-9" />
        </div>
        <Select value={roleFilter} onValueChange={(v) => { setRoleFilter(v as AppRole | 'todos'); setPage(0); }}>
      ...
      <div className="rounded-lg border border-border bg-card">
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow key={u.id}>
                ...
                <TableCell>
                  <Select value={u.role} onValueChange={(v) => roleMutation.mutate({ id: u.id, role: v as AppRole })}>
        ...
        <div className="flex items-center justify-between border-t border-border p-3 text-sm">
          <p className="text-muted-foreground">{filtered.length} usuário(s) · Página {page + 1}/{totalPages}</p>
...
function PermissoesSheet({ usuario, onClose }: { usuario: UsuarioPerfil | null; onClose: () => void }) {
  const qc = useQueryClient();
    ...
    <Sheet open={!!usuario} onOpenChange={(o) => !o && onClose()}>
      <SheetContent className="w-full sm:max-w-md">
        <SheetHeader>
          ...
          <SheetDescription>
            {usuario?.nome_completo} · <span className="text-foreground">{usuario && ROLE_LABELS[usuario.role]}</span>
          </SheetDescription>
        </SheetHeader>
        <div className="mt-6 space-y-4 px-4">
          {PERM_KEYS.map(({ key, label }) => {
            ...
            return (
              <div key={key} className="flex items-center justify-between rounded-md border border-border bg-secondary/30 p-3">
